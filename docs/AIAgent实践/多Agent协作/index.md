...
## 🔀 2026 多 Agent 五大编排模式：生产验证

### 为什么单 Agent 会撞墙

在讨论多 Agent 之前，先理清单 Agent 的四个硬限制：

| 限制 | 表现 | 架构影响 |
|------|------|---------|
| **上下文窗口** | 大代码库重构、多文档研究合成超出 200K token 窗口 | 要么截断（丢信息），要么外部化（需架构） |
| **技能专业化** | 通用 Agent 什么都做但什么都不精；微调过的代码审查 Agent 远胜通用模型 | 需要路由到专业 Agent |
| **并行瓶颈** | 如果三个子任务独立，顺序执行浪费 3 倍时间 | 并行 Agent 降低端到端延迟 |
| **可靠性冗余** | 高风险决策中单次推理不可靠 | 多数投票/辩论提升可靠性 |
...
#### 模式 1：Supervisor-Worker（主管-工人层级）

最直观的模式：编排 Agent 接收高层任务→分解→委派给专业 Worker →合成结果。

| 维度 | 评价 |
|------|------|
| **适用** | 任务有清晰子任务分解，主管能可靠识别该调用哪个 Agent |
| **失败条件** | 主管模型不够强做不好分解决策，或子任务高度相互依赖导致主管成为瓶颈不断来回路由信息 |
| **关键教训** | **主管的提示词和 Worker 的能力同样重要**。提示词不足的主管会把任务路由到错误 Agent，连锁错误贯穿整个管道 |

#### 模式 2：Peer-to-Peer（点对点对话，AutoGen 模型）

Agent 作为对话对等体直接交流，无需中心协调者。一个 Agent 提议→另一个批评→第一个修订。自然映射到代码审查、学术同行评审、编辑周期等人类工作流。

| 维度 | 评价 |
|------|------|
| **适用** | 开放式任务（分解方式不确定）、创意任务（批评-修订循环）、调试场景 |
| **失败条件** | 对话发散、Agent 各说各话（冗长、昂贵且循环）；没有终止条件时无限运行 |
| **关键教训** | **永远设置 max_turns 并实现显式完成信号** |

#### 模式 3：Map-Reduce（并行映射-归约）

适用于天然可并行的任务：处理文档列表、运行多个研究查询、评估数据集。Map 步骤将每份文档分发给独立 Worker →所有 Worker 完成后，Reduce 步骤收集结果并综合。

| 维度 | 评价 |
|------|------|
| **关键设计** | **Reduce 步骤的质量决定最终输出**。简单拼接很少好——需要综合 Agent 通读所有 Worker 输出、投票机制或结构化合并 |
| 实现 | LangGraph 的 `Send` 原语让此模式实现简洁 |

#### 模式 4：Debate/Ensemble（辩论/集成）

对高风险决策（医疗分诊、法律分析、金融建议），N 个 Agent 独立回答同一问题→互相看到对方输出→修订→投票/综合。

| 维度 | 评价 |
|------|------|
| **适用** | 有可验证答案的事实性问题、高风险分类决策、任何单模型过度自信构成风险的场景 |
| **失败条件** | 所有 Agent 共享相同盲点（相同基础模型、相同训练数据）。**集成的收益需要真正的多样性——不同模型、不同提示策略或不同信息源** |
| 证据 | Du et al. (2023) 证明 Agent 在看到他人推理后更新自身回答，在推理基准上超越单 Agent |

#### 模式 5：DAG Pipeline（有向无环图管道）

将任务建模为专用 Agent 的有向无环图，每个 Agent 转换状态并传递给下一个。比 Supervisor-Worker 系统的动态性低，但更可靠、更易测试。

典型研究管道：查询扩展 Agent →网页检索器→事实核查器→摘要器→格式化器。

| 维度 | 评价 |
|------|------|
| **适用** | 理解透彻、可重复的工作流：文档处理、数据提取、报告生成 |
| **失败条件** | 管道结构依赖输入本身的任务（需要动态分解） |
| **关键优势** | 每个节点可独立单元测试，整个管道可集成测试 |

### 选择启发式

```
已知的、稳定的分解 → DAG Pipeline
动态分解 → Supervisor-Worker
迭代精炼 → Peer-to-Peer 对话
可并行子任务 → Map-Reduce
