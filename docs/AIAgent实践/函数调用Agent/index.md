...
## 📊 2026 函数调用评测标准：BFCL v4 与 tau-Bench

### 为什么通用评测会误导 Agent 团队

2026 年，一个关键认知已经形成：**MMLU、MT-Bench 和 HumanEval 都无法衡量工具调用 Agent 的核心能力**。模型可以在 MMLU 上达到 90 分位，但在复杂嵌套工具 schema 上仍有 **12% 的失败率**——选错函数、填错参数类型、幻觉出不存在的函数。这些失败不会出现在对话质量评分中。

> 来源：[Spheron Blog — AI Agent Tool Calling Benchmarks (Jun 2026)](https://www.spheron.network/blog/tool-calling-benchmarks-bfcl-tau-bench-latency-optimization/)

| 评测 | 测量内容 | Agent 相关性 |
|------|---------|------------|
| MMLU | 事实性知识（选择题） | 低——Agent 不做选择题 |
| MT-Bench | 多轮对话质量 | 中——测对话，不测工具调用 |
| HumanEval | Python 代码生成 | 对工具调用专项低 |
| **BFCL v4** | 函数选择 + 参数填充准确率 | **高** |
| **tau-Bench** | 端到端多轮任务完成率 | **非常高** |

### BFCL v4（Berkeley Function-Calling Leaderboard v4）

v4 版本（2026 年 4 月发布）转向了**整体 Agent 评估模型**，覆盖五大领域共 2,000+ 测试用例：

| 评估维度 | 权重 | 说明 |
|---------|------|------|
| **Agentic（多步任务）** | 40% | 模型必须规划和执行一系列工具调用完成端到端目标 |
| **Multi-Turn（多轮）** | 30% | 跨多轮对话保持准确的函数调用上下文，追踪先前结果和用户纠正 |
| **Live（实时）** | 10% | 来自开发者提交到排行榜的真实 API 调用案例 |
| **Non-Live（静态）** | 10% | 从简单单参数到复杂嵌套多工具 schema 的精选测试用例 |
| **Hallucination（幻觉检测）** | 10% | 用户查询与可用工具不匹配时，模型是否正确**拒绝调用** |

### tau-Bench：端到端任务完成

tau-Bench 不测量单次调用准确率，而是**端到端任务完成率**。模型面对模拟用户进行多轮 Agent 对话，需要完成如"取消 3 月 3 日的订单并将退款转为商店积分"这类任务——涉及多步工具调用、处理用户中途纠正、判断任务何时真正完成。

- 约 115 个零售任务 + 50 个航空任务
- 主要指标：pass@1 任务完成率（完全正确才算通过）
- 比 BFCL v4 更难：模型可能在单次调用上正确，但在跨 3 轮后丢失上下文仍会失败

### 2026 模型工具调用排行榜

| 模型 | BFCL v4 总得分 | 单轮 | 多轮 | 备注 |
|------|--------------|------|------|------|
| GPT-4o | ~82% | ~86% | ~78% | 闭源 API |
| Claude Sonnet 4 | ~79% | ~83% | ~74% | 开源，可自部署 |
| DeepSeek V3 | ~78% | ~82% | ~73% | 开源，可自部署 |
| Mistral Small 4 | ~72% | ~76% | ~68% | 开源，可自部署 |
| Llama 4 Scout 17B | ~68% | ~72% | ~63% | 开源，紧凑型 |

**两个关键发现**：

1. **开源已追平闭源**：顶级闭源 API 与顶级开源模型之间的差距已缩小到 3-4 个百分点，自部署 Qwen 2.5 72B 或 DeepSeek V3 在大多数生产场景中足够
2. **多轮退化是普遍规律**：每个模型的多轮得分比单轮低 5-10 个百分点。如果你的 Agent 每个任务需要 5+ 次顺序工具调用，**你应该关心的有效准确率是多轮得分的复合结果，而非头条数字**

### 自部署函数调用的延迟优化

函数调用解码与普通聊天生成有三个结构性差异：

| 延迟来源 | 普通生成 | 函数调用额外开销 | 优化方案 |
|---------|---------|----------------|---------|
| Prompt Prefill | 与上下文成正比 | +400-800 tokens（工具 schema） | `--enable-prefix-caching` KV 缓存复用 |
| TTFT | 基线 | +2-5s（冷语法编译 + schema prefill） | xgrammar + 语法缓存 |
| 逐 Token 解码 | 基线 | +3-8%（token 掩码开销） | xgrammar 将掩码开销降至 ~0% |
...
关键标志说明：
- `--enable-auto-tool-choice`：激活工具感知生成，vLLM 处理模型原生函数调用格式的模板渲染
- `--tool-call-parser hermes`：将模型原生工具调用响应解析为 OpenAI API 格式
- `--enable-prefix-caching`：工具 schema 的 KV 缓存复用——首次请求支付完整 schema prefill 成本，后续同 system+tools 前缀的调用命中缓存

> 来源：[Spheron Blog — AI Agent Tool Calling Benchmarks on GPU Cloud (Jun 2026)](https://www.spheron.network/blog/tool-calling-benchmarks-bfcl-tau-bench-latency-optimization/)
