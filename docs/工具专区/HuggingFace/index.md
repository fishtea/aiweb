...
### 🤗 Transformers v5.14.0 — Inkling 975B 多模态模型与 MTP 解码（2026-07-15）

2026年7月15日，Transformers 发布 **v5.14.0**，带来多项重大更新。

#### Inkling（Thinking Machines）— 975B 参数开源多模态 MoE

**Inkling** 是 Thinking Machines 发布的新一代开源多模态模型（Apache 2.0），总参数量 **975B**、激活 41B（MoE 架构），支持文本、图像和音频输入，生成文本输出：

| 属性 | 详情 |
|------|------|
| **参数量** | 975B 总参数 / 41B 激活参数 |
| **架构** | MoE（混合专家） |
| **输入** | 文本 + 图像 + 音频 |
| **输出** | 文本 |
| **许可** | Apache 2.0 |
| **适用场景** | Agent 系统、编程助手、对话、RAG、多模态任务 |
| **HuggingFace 页面** | [thinkingmachines/Inkling](https://huggingface.co/thinkingmachines/Inkling) |

Inkling 在 HuggingFace 上已获得 **1,500+ 点赞**，并提供 NVFP4 量化版（10 万+ 下载）和 GGUF 版（5 万+ 下载），适合不同硬件配置。

#### TIPSv2 视觉模型系列

新增 **TIPSv2** 和 **TIPSv2 DPT** 两个视觉模型架构：

- **TIPSv2**：新一代 Transformer 图像处理架构
- **TIPSv2 DPT**：基于 TIPSv2 的深度估计模型（Dense Prediction Transformer）

#### Multi-Token Prediction (MTP) 解码支持

这是 v5.14.0 生成模块最重要的改进——**原生 MTP 解码支持**：

- 模型可在单次前向传播中预测多个未来 token，而非逐个生成
- 与推测解码（Speculative Decoding）结合时，通过静态集成验证（Static Ensemble Verification）提升草案 token 接受率
- 大幅降低解码延迟，特别适合需要快速生成长文本的场景

#### SDPA Prefill + FlashAttention 加速

SDPA Prefill 更新为使用 FlashAttention 内核配合 `StaticCache`，对于大输入规模性能提升高达 **260%**。

#### Breaking Changes

- **GPTNeoX**：`embed_out` 权重重映射至 `lm_head`
- **GPTBigCode**：默认启用 `_supports_attention_backend = True`，提升 vLLM 兼容性

### 参考来源

- [Transformers v5.14.0 Release Notes](https://github.com/huggingface/transformers/releases/tag/v5.14.0)
- [thinkingmachines/Inkling - HuggingFace](https://huggingface.co/thinkingmachines/Inkling)
- [Inkling NVFP4 - HuggingFace](https://huggingface.co/thinkingmachines/Inkling-NVFP4)
