...
## 2026 年 7 月更新：Gemini 3.6 Flash 与 3.5 Flash-Lite

根据 [9to5Google 报道](https://9to5google.com/2026/07/21/gemini-3-6-flash-launch/)（2026 年 7 月 21 日），Google 发布了 Gemini 3.6 Flash 和 3.5 Flash-Lite：

### Gemini 3.6 Flash

基于 2026 年 I/O 发布的 3.5 Flash 进行优化，3.6 Flash 根据开发者和客户反馈做了多项改进：

| 维度 | Gemini 3.5 Flash | Gemini 3.6 Flash | 变化 |
|------|-----------------|------------------|------|
| 输出 token 效率 | 基准 | **减少 17% 输出 token** | 更省成本 |
| 推理步骤 | 基准 | 更少步骤完成多步工作流 | 更快 |
| 定价（输出） | $9/1M tokens | **$7.50/1M tokens** | ↓17% |
| DeepSWE（编码） | 37% | **49%** | +12pp |
| MLE Bench（ML 研究） | 49.7% | **63.9%** | +14.2pp |
| GDPval-AA（知识工作） | 1,349 | **1,421** | +72 |
| OSWorld-Verified（计算机使用） | 78.4% | **83%** | +4.6pp |
| 知识截止日期 | 2025 年 1 月 | **2026 年 3 月** | 更新 14 个月 |

### Gemini 3.5 Flash-Lite

面向高吞吐、低延迟场景（如 Agent 搜索和文档处理）：

| 维度 | 3.1 Flash-Lite | 3.5 Flash-Lite | 提升 |
|------|---------------|----------------|------|
| 定价（输入/输出） | — | **$0.30 / $2.50** / 1M tokens | 极致性价比 |
| Terminal-Bench 2.1 | 31% | **54%** | +23pp |
| SWE-Bench Pro | 49.6% (vs 3 Flash) | **54.2%** | 超越 3 Flash |
| OSWorld-Verified | 65.1% (vs 3 Flash) | **74.0%** | 超越 3 Flash |
| GDM-MRCR v2（长上下文） | 60.1% | **72.2%** | +12.1pp |
| GDPval-AA v2 | 642 | **1,140** | +498 |

> Flash-Lite 以 **$0.30 / $2.50** 的定价，在编码和 Agent 任务上已超越 Gemini 3 Flash。非常适合大规模文档处理、Agent 搜索和批量推理场景。

### Gemini 3.5 Flash Cyber

同时发布的 **Gemini 3.5 Flash Cyber** 专注于发现和修复安全漏洞，支持检测、验证和修补代码安全问题。Google 的 CodeMender 工具使用多个 3.5 Flash Cyber Agent 协同工作。目前以有限试点形式向政府和可信合作伙伴开放。

## Gemini 2.5 Flash Image：原生图像生成与编辑

根据 [Google Developers Blog](https://developers.googleblog.com/en/introducing-gemini-2-5-flash-image/)（2026 年），Gemini 2.5 Flash Image（代号 nano-banana）是最新的图像生成与编辑模型：
...
|------|------|
| **多图融合** | 将多张图像混合生成为单一图像 |
| **角色一致性** | 在多轮生成中保持角色形象一致，适合故事创作 |
| **自然语言编辑** | 用自然语言描述对图像进行定向修改 |
| **世界知识驱动** | 利用 Gemini 的世界知识生成和编辑图像 |
| **低延迟高性价比** | 基于 2.5 Flash 架构，速度快、成本低 |

### 定价与获取

- 定价：**$30.00 / 1M 输出 tokens**，每张图约 1,290 输出 tokens（约 **$0.039/张**）
- 获取方式：Gemini API、Google AI Studio（开发者）、Vertex AI（企业）
- 输入和其他模态按 Gemini 2.5 Flash 标准定价

> **与 3.x Flash 的关系**：2.5 Flash Image 是专门的图像生成模型，与 3.6 Flash（文本/推理为主）互补。实际应用中可组合使用：3.6 Flash 做文本理解和 Agent 调度，2.5 Flash Image 做图像生成。

## 2026 下半年 Gemini 展望

- **Gemini 3.5 Pro** 正在与合作伙伴测试中，即将广泛发布
- **Gemini 4** 已被 Google 提及正在研发中（"building the next generation"）
- Managed Agents 生态持续扩展：后台任务、远程 MCP、生产级可观测性

### 2026 年 7 月 API 价格战与 Gemini 的竞争定位

2026 年 6-7 月，DeepSeek-V4 以 $0.435/1M input（Pro）和 $0.14/1M input（Flash）的激进定价引发了一场 API 价格战。Google 的回应迅速而有力：

| 事件 | 日期 | 影响 |
|------|------|------|
| DeepSeek-V4 发布 | 2026-06-27 | $0.435 input，LiveCodeBench 93.5%，MIT 开源 |
| Gemini 3.6 Flash 发布 | 2026-07-21 | 输出降价 17% 至 $7.50，性能显著提升 |
| Gemini 3.5 Flash-Lite 发布 | 2026-07-21 | $0.30 input / $2.50 output，大规模处理场景极具竞争力 |

**竞争格局分析：**

- **超低价区间**：Gemini 3.5 Flash-Lite（$0.30）vs DeepSeek-V4-Flash（$0.14）——DeepSeek 在纯文本推理上仍更便宜，但 Gemini 有原生多模态优势
- **中端性价比**：Gemini 3.6 Flash（$4.50/$7.50）vs DeepSeek-V4-Pro non-think（$0.435/$0.87）——DeepSeek 价格优势明显，但 Gemini 的多模态能力独一无二
- **旗舰推理**：Gemini 3 Pro（$8.00/$24.00）vs DeepSeek-V4-Pro think（$0.435/$0.87）——DeepSeek 在代码/数学推理上分数更高且价格更低

> **结论**：纯文本推理和代码场景，DeepSeek-V4 性价比领先；需要原生多模态（视频/音频/图像理解）或 Google 生态集成（Search/Maps/Calendar）时，Gemini 无可替代。

### Ollama 对 Gemini 模型的支持进展

Ollama [v0.32.1](https://github.com/ollama/ollama/releases)（2026-07-16）改进了 **Gemma 4 的工具调用和多轮推理能力**，修复了 tool-response continuation 的可靠性问题，并解决了 MLX 模型缓存泄漏导致的内存增长。这意味着在 Apple Silicon Mac 上本地运行 Gemma 系列模型进行 Agent 开发的体验进一步提升。

### Gemini 3.5 Pro 前瞻

根据 Google 在 2026 年 7 月的公开信息，Gemini 3.5 Pro 目前处于与合作伙伴的测试阶段，预计将在 2026 年下半年广泛发布。基于 3.6 Flash 的改进方向，3.5 Pro 预计将带来：
- 超越 2.5 Pro 的推理深度和知识广度
- 与 Managed Agents 生态的更深度整合
- 可能引入更长上下文或更高效的注意力机制

### Google 的 Agent 生态护城河

与 OpenAI 和 Anthropic 不同，Google 的差异化优势在于 **端到端 Agent 基础设施**：

```
Google Agent 栈
├── 模型层: Gemini 3 Pro / 3.6 Flash / 3.5 Flash-Lite
├── Agent 框架: Managed Agents（后台任务 + 远程 MCP）
├── 工具层: Google Search / Maps / Calendar / Gmail 原生集成
├── 部署层: Vertex AI Agent Builder（企业安全 + SLA）
└── 硬件层: TPU v5 / v6 推理加速
```

这种垂直整合意味着使用 Gemini 构建 Agent 时，搜索 grounding、地图定位、日历调度等能力是原生的，无需额外的 API 编排。

---

## 2026 年 7 月补充：生产部署与实践指南

### Gemini Audio 模型

根据 [Google DeepMind 博客](https://deepmind.google/discover/blog/)（2026 年 7 月），**Gemini Audio** 是 Google 最新推出的语音原生模型，支持语音对话、音频创作和语音控制。与早期语音方案（通过 ASR + TTS 拼凑）不同，Gemini Audio 直接理解和生成音频，实现了更自然的语音交互体验。目前通过 Gemini API 和 Google AI Studio 提供。

### 模型选型与定价对比

截至 2026 年 7 月，Gemini API 各主要模型的定价如下（窗口期为 2026 年 7 月，来源为 Google AI for Developers）：

| 模型 | 输入价格 ($/1M tokens) | 输出价格 ($/1M tokens) | 上下文 | 最适合场景 |
|------|----------------------|----------------------|--------|-----------|
| **Gemini 3 Pro** | $8.00 | $24.00 | 1M+ | 旗舰推理、复杂多模态 |
| **Gemini 2.5 Pro** | $8.00 | $24.00 | 1M+ | 稳定推理（已有管线推荐） |
| **Gemini 3.6 Flash** | $4.50 | $7.50 | 1M | 新一代性价比推理+Agent |
| **Gemini 3.5 Flash** | $4.50 | $9.00 | 1M | 均衡型 Agent 任务 |
| **Gemini 3.5 Flash-Lite** | $0.30 | $2.50 | 1M | 大吞吐文档处理、搜索 |
| **Gemini 2.0 Flash** | $3.00 | $6.00 | 1M | 对延迟敏感的低成本任务 |
| **Gemini 2.5 Flash Image** | （按 Flash 定价） | $30.00 | — | 图像生成与编辑 |
| **Gemini Embedding** | $0.05 | — | — | RAG、语义搜索 |

> 选型建议：新项目优先考虑 **Gemini 3.6 Flash**（性价比+能力最佳平衡）或 **Gemini 3 Pro**（高难度推理）。对大规模批量任务，**3.5 Flash-Lite** 以 $0.30 的输入价格极具竞争力。

### 生产部署最佳实践

**1. API 调用优化**
- **流式输出（Streaming）**：对用户体验敏感的场景（对话、代码补全）始终启用 `stream: true`，首 token 延迟可降低 60-80%
- **结构化输出**：使用 `response_mime_type: "application/json"` 获得 JSON 格式的结构化响应，配合 `response_schema` 参数约束输出格式
- **Thinking 模式控制**：对简单问题关闭 Thinking、对复杂推理调用开启，平衡成本与质量

**2. 上下文窗口利用**
- 利用 Gemini 的超长上下文（1M tokens）减少 RAG 系统的分段复杂度
- 对于 <100K tokens 的知识库，可直接全量塞入上下文，省去向量检索环节
- 注意 Lost-in-the-Middle 现象：关键信息放在上下文的开头和末尾

**3. Agent 架构选择**
- **Managed Agents（托管 Agent）**：适合生产级 Agent 系统，提供后台任务、远程 MCP、状态管理和重试机制
- **Google AI Edge SDK**：适合端侧部署和实时应用，支持模型量化和本地推理
- **Vertex AI Agent Builder**：适合企业级部署，提供企业安全、合规和 SLA 保障
